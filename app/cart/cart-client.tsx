...
export function CartClient(){
  ...
  const [availability,setAvailability]=useState<{product:{id:string;name:string;image?:string};quantity:number}|null>(null);
  ...
  return <><div className="cart-layout">
    <section className="cart-items"><p className="eyebrow">Your selection</p><h1>Cart</h1>
      ...
      {items.map(item=><article className="cart-row" key={item.id}>
        ...
        <div><Link href={`/products/${item.slug}`}><b>{item.name}</b></Link><small>{money(item.price)} each</small><button className="cart-availability-link" onClick={()=>setAvailability({product:item,quantity:item.quantity})}>Check availability</button></div>
    ...
    <aside className="cart-summary"><p className="eyebrow">Price summary</p><div><span>Subtotal</span><b>{money(subtotal)}</b></div><div><span>Ontario HST (13%)</span><b>{money(tax)}</b></div><div className="cart-total"><span>Estimated total</span><strong>{money(total)}</strong></div>
      <button className="cart-primary-action" onClick={()=>setAvailability({product:{id:items[0].id,name:`Cart request: ${items.map(item=>`${item.quantity}× ${item.name}`).join(", ")}`,image:items[0].image},quantity:items.reduce((sum,item)=>sum+item.quantity,0)})}>Check availability for this cart</button>
  ...
  </div>{availability&&<Inquiry product={availability.product} initialQuantity={availability.quantity} close={()=>setAvailability(null)}/>}</>;
}
