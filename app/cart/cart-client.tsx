...
export function CartClient(){
  ...
  if(!items.length)return <div className="cart-empty"><span>0</span><h1>Your list is empty</h1><p>Save products here, then ask our Barrie store to confirm availability.</p><Link className="primary" href="/#catalogue">Browse products</Link></div>;
  return <div className="cart-layout">
    <section className="cart-items"><p className="eyebrow">Your selection</p><h1>My List</h1>
      ...
      {items.map(item=><article className="cart-row" key={item.id}>
        ...
        <div><Link href={`/products/${item.slug}`}><b>{item.name}</b></Link><small>{money(item.price)} each</small><Link className="list-availability-link" href={`/products/${item.slug}`}>Check availability</Link></div>
    ...
    <aside className="cart-summary"><p className="eyebrow">Price summary</p><div><span>Subtotal</span><b>{money(subtotal)}</b></div><div><span>Ontario HST (13%)</span><b>{money(tax)}</b></div><div className="cart-total"><span>Estimated total</span><strong>{money(total)}</strong></div>
      <Link className="list-primary-link" href={`/products/${items[0].slug}`}>Check availability</Link>
      <p>Availability and current price are confirmed by the store. Your list is an inquiry aid and does not reserve products.</p>
