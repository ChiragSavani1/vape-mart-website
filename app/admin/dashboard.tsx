...
export function AdminDashboard({
  ...
  async function uploadBanner(form:HTMLFormElement){
    ...
    try{
      ...
      for(const name of ["headline","subtitle","label","ctaText","ctaUrl"]){const field=form.elements.namedItem(name) as HTMLInputElement|null;if(field)body.append(name,field.value)}
  ...
  async function updateBanner(banner:SiteBanner,patch:Partial<SiteBanner>){
    setRequestError("");const response=await fetch(`/api/admin/banners/${banner.id}`,{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({...banner,...patch})}),data=await response.json().catch(()=>null);
    if(response.ok)setBanners(data.banners);else setRequestError(data?.error||"The banner details could not be saved.");
  }
  ...
  return <div className="admin-shell">
    ...
    <main className="admin-main"><header className="admin-top"><div><p className="eyebrow">Vape Mart admin</p><h1>{tab}</h1></div><div><Link href="/" target="_blank">View catalogue ↗</Link><button className="admin-primary" onClick={()=>{setTab("Products");setEditing({visible:true,featured:false,price:0})}}>+ Add product</button></div></header>
      ...
      {tab==="Banners"&&<><Panel title={replacementBannerId?"Replace missing hero banner":`Hero banners (${banners.length}/6)`}><form className="banner-upload" onSubmit={event=>{event.preventDefault();uploadBanner(event.currentTarget)}}><label>Banner image<input name="file" type="file" accept=".png,.jpg,.jpeg,.webp,image/png,image/jpeg,image/webp" required disabled={banners.length>=6}/></label><label>Accessible description<input name="alt" placeholder="New arrival promotion" required/></label><label>Label<input name="label" placeholder="New arrival"/></label><label>Headline<input name="headline" placeholder="A short promotional headline"/></label><label>Subtitle<input name="subtitle" placeholder="Optional supporting text"/></label><label>Button text<input name="ctaText" placeholder="Shop the collection"/></label><label>Button URL<input name="ctaUrl" defaultValue="/#catalogue"/></label><button className="admin-primary" disabled={uploading||banners.length>=6}>{uploading?"Uploading…":replacementBannerId?"Upload replacement":"Upload banner"}</button>{replacementBannerId&&<button type="button" className="table-action" onClick={()=>setReplacementBannerId("")}>Cancel replacement</button>}</form><p className="panel-intro">{banners.length?"PNG, JPG, and WebP banners are optimized and shown immediately, then included in the next GitHub archive action.":"No custom banners uploaded. The original three banners are currently shown."}</p><div className="banner-admin-grid">{banners.map(banner=><article key={banner.id}><img src={banner.src} alt=""/><div><b>{banner.headline||banner.alt}</b><small>{banner.label||"Hero banner"} · Position {banner.position+1}</small><label className="banner-active"><input type="checkbox" checked={banner.active} onChange={event=>updateBanner(banner,{active:event.target.checked})}/> Active</label><div className="banner-card-actions"><button disabled={banner.position<=0} onClick={()=>updateBanner(banner,{position:banner.position-1})}>Move up</button><button disabled={banner.position>=5} onClick={()=>updateBanner(banner,{position:banner.position+1})}>Move down</button><button onClick={()=>deleteBanner(banner.id)}>Remove</button></div></div></article>)}</div></Panel></>}
