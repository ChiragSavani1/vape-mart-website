...
export function AdminDashboard({
  ...
  return <div className="admin-shell">
    ...
    <main className="admin-main"><header className="admin-top"><div><p className="eyebrow">Vape Mart admin</p><h1>{tab}</h1></div><div><Link href="/" target="_blank">View catalogue ↗</Link><button className="admin-primary">+ Add product</button></div></header>
      ...
      {tab==="Images"&&<Panel title="Distributor image review" actionLabel="Sync Dropbox sources"><p className="panel-intro">UPC matches are prioritized. Name-based matches use brand, series, flavour, puff count, and normalized product names. Approved files are copied to site storage.</p><div className="image-review">{products.slice(0,4).map((p)=><div className="match-card" key={p.id}><div className="match-pair"><span style={{background:p.accent}}>Placeholder</span><span style={{background:"#dedede",color:"#333"}}>Awaiting image</span></div><h3>{p.name}</h3><p>No approved distributor match</p><div className="confidence"><i style={{width:"0%"}}></i></div><b>0% confidence</b><div><button disabled>Approve</button><button>Keep placeholder</button><button>Select manually</button></div></div>)}</div></Panel>}
      {tab==="Promotions"&&<div className="two-columns"><Panel title="Promotion setup"><form className="admin-form" onSubmit={e=>{e.preventDefault();setPromotion(true)}}><label>Promotion name<input defaultValue="Summer flavour feature"/></label><label>Promotional banner<input type="file" accept="image/*"/></label><label>Participating products<select multiple defaultValue={["1","2"]}>{products.map(p=><option value={p.id} key={p.id}>{p.name}</option>)}</select></label><div className="form-row"><label>Promotional price<input type="number" step=".01" defaultValue="29.99"/></label><label>Starts<input type="date"/></label><label>Expires<input type="date"/></label></div><label className="check"><input type="checkbox" required/> I approve this promotional asset for publishing</label><button className="admin-primary">Save promotion</button>{promotion&&<p className="save-note">Promotion saved. It will activate and expire automatically.</p>}</form></Panel><Panel title="Promotion rules"><div className="health-list"><Health label="Schedule" value="Automatic"/><Health label="Asset approval" value="Required"/><Health label="Expired price" value="Reverts automatically" good/><Health label="Online checkout" value="Disabled" good/></div></Panel></div>}
      {tab==="Requests"&&<Panel title="Availability requests"><RequestTable rows={requests} status={status}/></Panel>}
      {tab==="Content"&&<Panel title="Homepage & warning content"><form className="admin-form"><label>Homepage headline<input defaultValue="Find your flavour. Check it in store."/></label><label>Homepage banner<textarea defaultValue="Featured flavours. Limited-time prices."/></label><label>Age gate text<textarea defaultValue="You must be of legal age in Ontario to view this website."/></label><label>Canada / Ontario warning<textarea defaultValue="Vaping products contain nicotine. Nicotine is highly addictive. Adults 19+ only."/></label><button className="admin-primary" type="button">Save content</button></form></Panel>}
      {tab==="Store"&&<Panel title="Store information"><form className="admin-form"><label>Store name<input defaultValue="Vape Mart"/></label><label>Address<input defaultValue="307 Cundles Road East, Barrie, ON"/></label><div className="form-row"><label>Phone<input defaultValue="(705) 721-8181"/></label><label>Email<input defaultValue="vapemart307@gmail.com"/></label></div><label>Opening hours<textarea defaultValue={"Mon–Fri: 9 AM–10 PM\nSat: 10 AM–8 PM\nSun: 10 AM–9 PM"}/></label><button className="admin-primary" type="button">Save store information</button></form></Panel>}
    </main>
  </div>
}
