...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><Header/><main className="subpage"><div className="subpage-wrap detail">
    ...
    <div className="detail-copy"><p className="eyebrow">{product.brand} · {product.category}</p><h1>{product.name}</h1>
      ...
      <div className="detail-facts"><div><small>Flavour</small><b>{product.flavour}</b></div><div><small>Category</small><b>{product.category}</b></div><div><small>UPC</small><b>{product.upc}</b></div></div>
