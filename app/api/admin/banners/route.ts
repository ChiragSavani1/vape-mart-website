...
export async function POST(request:NextRequest){
  ...
  try{
    ...
    if(replacement) {
      await db.prepare(`UPDATE banners SET object_key=?,alt_text=?,original_filename=?,temporary_path=?,public_url=?,
        ...
        .bind(`temporary/${id}.webp`,alt,temporary.originalFilename,temporary.filePath,temporary.url,now,id).run();
    } else {
      await db.prepare(`INSERT INTO banners
        (id,object_key,alt_text,position,created_at,original_filename,temporary_path,public_url,image_status,source_type,updated_at)
        VALUES (?,?,?,?,?,?,?,?,?,?,?)`)
        .bind(id,`temporary/${id}.webp`,alt,count?.count||0,now,temporary.originalFilename,temporary.filePath,
