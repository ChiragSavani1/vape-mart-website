...
export async function PATCH(request:NextRequest,{params}:{params:Promise<{id:string}>}){
  ...
  if(!["Available","Unavailable"].includes(status))return NextResponse.json({error:"Invalid status"},{status:400});
  ...
  await db.prepare("UPDATE inquiries SET status = ?, responded_at = ? WHERE id = ?").bind(status,new Date().toISOString(),id).run();
  if(inquiry.contact.includes("@")&&process.env.RESEND_API_KEY){
